...
function fuzzySearch(text: string | null | undefined, query: string): boolean {
  ...
  if (
    (textLower.length < 20 && textLower.includes(" ")) ||
    ...
    for (const queryWord of queryWords) {
      ...
      for (const textWord of textWords) {
        ...
        if (
          queryWord.length >= 3 &&
          textWord.startsWith(queryWord.substring(0, 3))
        ) {
          return true;
        }
  ...
  if (
    queryLower.includes("a-level") ||
    queryLower.includes("alevel") ||
    textLower.includes("a-level") ||
    textLower.includes("alevel")
  ) {
    ...
    if (
      normalizedQuery.includes("alevel") &&
      !normalizedText.includes("alevel")
    ) {
  ...
  const queryInitials = queryLower
    .split(/\s+/)
    .map((word) => word[0] || "")
    .join("");
  const textWords = textLower.split(/\s+/);
  ...
  if (
    queryLower.length <= 5 &&
    queryLower.length > 1 &&
    textInitials.includes(queryLower)
  ) {
    return true;
  }
  ...
  for (const word of queryWords) {
    ...
    for (const textWord of textWords) {
      ...
      if (
        word.length >= 4 &&
        textWord.startsWith(word.substring(0, prefixLength))
      )
        return true;
    ...
    for (const textWord of textWords) {
      if (textWord.length >= 5 && word.length >= 4) {
        ...
        if (textWord.endsWith(word.substring(word.length - suffixLength)))
          return true;
      }
    }
    ...
    for (const textWord of textWords) {
      if (textWord.length > 3 && word.length > 3) {
        ...
        if (calculateSimilarity(textWord, word) > similarityThreshold)
          return true;
      }
    }
  }
...
function scoreSearchRelevance(
  ...
  if (
    searchTermLower.includes("university") ||
  ...
  if (
    tutorFields.major &&
    tutorFields.major.toLowerCase().includes(searchTermLower)
  ) {
...
const extractUniversityName = (
  education: string | string[] | null | undefined
): string | null => {
  ...
  // Direct check for UCLA
  if (
    educationText.includes("UCLA") ||
    educationText
      .toLowerCase()
      .includes("university of california, los angeles") ||
  ...
  if (educationText.includes("University of California")) {
    ...
    const matches = educationText.match(
      /University of California,?\s+(?:[A-Za-z]+\s*)+/i
    );
...
const getCountryCodes = (
  countryArray: string[] | null | undefined
): string[] => {
...
export default function TutorsPage() {
  ...
  const [sortOrder, setSortOrder] = useState<"rating" | "name" | "popularity">(
    "rating"
  );
  const [tutorRatings, setTutorRatings] = useState<{
    [key: string]: TutorRating;
  }>({});
  ...
  const filteredTutors = tutors.filter((tutor: TutorProfile) => {
    ...
    const tutorExtracurriculars: string[] = extractSearchableText(
    ...
    if (
      tutor.major &&
      typeof tutor.major === "string" &&
      tutor.major.trim().length > 0
    ) {
    ...
    if (
      tutor.year &&
      typeof tutor.year === "string" &&
      tutor.year.trim().length > 0
    ) {
    ...
    if (tutor.major && tutor.current_education) {
      ...
      const majorText =
        typeof tutor.major === "string" ? tutor.major.trim() : "";

      if (majorText) {
        if (typeof tutor.current_education === "string") {
          searchableKeywords.push(
            `${majorText} at ${tutor.current_education.trim()}`
          );
        } else if (Array.isArray(tutor.current_education)) {
    ...
    const matchesUniversities =
      ...
      tutorUniversities.some((university) =>
        ...
        <div className="flex flex-wrap items-center justify-between mb-8 gap-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <DropdownMenu
              ...
              onOpenChange={setIsSubjectFilterOpen}
            >
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  className="flex items-center gap-2 shadow-sm border-[#84b4cc]/40 hover:bg-[#c7e4e3]/10 transition-colors"
                >
                  <Filter
                    className="h-4 w-4 text-[#3e5461]"
                    strokeWidth={1.5}
                  />
                  Services{" "}
                  {selectedSubjects.length > 0 && (
                    <Badge
                      variant="secondary"
                      className="ml-1 bg-[#3e5461]/10 text-[#128ca0] border-none"
                    >
            ...
            <DropdownMenu
              open={isSchoolFilterOpen}
              onOpenChange={setIsSchoolFilterOpen}
            >
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  className="flex items-center gap-2 shadow-sm border-[#84b7bd]/40 hover:bg-[#c2d8d2]/10 transition-colors"
                >
                  <School
                    className="h-4 w-4 text-[#4b92a9]"
                    strokeWidth={1.5}
                  />
                  Schools{" "}
                  {selectedSchools.length > 0 && (
                    <Badge
                      variant="secondary"
                      className="ml-1 bg-[#4b92a9]/10 text-[#126d94] border-none"
                    >
            ...
            <DropdownMenu
              open={isUniversityFilterOpen}
              onOpenChange={setIsUniversityFilterOpen}
            >
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
