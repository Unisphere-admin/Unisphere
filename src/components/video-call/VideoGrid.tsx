...
const VideoGrid: React.FC = () => {
  ...
  // Play local video
  useEffect(() => {
    if (localVideoRef.current && localVideoTrack && !isVideoMuted && !isScreenSharing) {
      try {
  ...
  useEffect(() => {
    if (!isScreenSharing && localVideoRef.current && localVideoTrack && !isVideoMuted) {
      ...
      <div className="h-full flex items-center justify-center">
        <div className="relative w-full max-w-4xl aspect-video bg-[#222222] rounded-lg overflow-hidden">
          ...
          {isScreenSharing && screenTrack && (
            <div ref={screenShareRef} className="absolute inset-0 w-full h-full" />
          )}
    ...
    <div className="h-full">
      <div className="grid grid-cols-2 gap-4 h-full">
        ...
        {remoteUsers.slice(0, 3).map((remoteUser, index) => (
          <div key={remoteUser.uid} className="bg-[#222222] rounded-lg overflow-hidden relative min-h-0">
            {/* Remote video element - always rendered */}
            <div id={`remote-video-${remoteUser.uid}`} className="w-full h-full" />
          ...
          <div className="bg-[#222222] rounded-lg overflow-hidden flex items-center justify-center min-h-0">
            <div className="text-center text-white p-4">
              <div className="w-24 h-24 bg-[#333333] rounded-full flex items-center justify-center mx-auto mb-2 flex-shrink-0">
                <span className="text-2xl text-[#808080]">+{remoteUsers.length - 3}</span>
              </div>
