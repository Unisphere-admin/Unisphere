...
export const MessageProvider = ({
  children,
  pageVisibility: propPageVisibility,
}: {
  children: ReactNode;
  pageVisibility?: PageVisibility;
}) => {
  ...
  const messagesCacheRef = useRef<{
    [conversationId: string]: { data: Message[]; timestamp: number };
  }>({});
      ...
      try {
        ...
        if (stored) {
          ...
          if (
            ...
            Object.keys(cleanedConversations).length
          ) {
            localStorage.setItem(
              "tempConversations",
              JSON.stringify(cleanedConversations)
            );
          }
  ...
  const isTempConversation = useCallback(
    (conversationId: string) => {
      return (
        conversationId.startsWith("temp-") &&
        Boolean(tempConversations[conversationId])
      );
    },
    [tempConversations]
  );
  ...
  const createTempConversation = useCallback(
    (tutorId: string, tutorName: string, tutorAvatar?: string | null) => {
      ...
      const existingTempConv = Object.entries(tempConversations).find(
        ([_, conv]) => conv.tutorId === tutorId
      );
  ...
  const hasUnreadMessages = useCallback(
    (conversationId: string) => {
      ...
      if (conversation.last_message && lastViewedAt) {
        ...
        if (
          lastMessageTime > lastViewedTime &&
          conversation.last_message.sender_id !== user?.id
        ) {
          return true;
        }
  ...
  const verifyConversationReady = async (
    conversationId: string
  ): Promise<boolean> => {
  ...
  const updateMessageStatus = useCallback(
    (message: Message, status?: "sending" | "sent" | "delivered" | "error") => {
  ...
  const sortConversations = useCallback(
    (convos: Conversation[]): Conversation[] => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        const cachedConversations = getFromCache<Conversation[]>(
        ...
        if (cachedConversations && cachedConversations.length > 0) {
          ...
          if (tempConvs.length > 0) {
            setConversations(
              sortConversations([...tempConvs, ...cachedConversations])
            );
          } else {
    ...
    const fetchFreshConversations = async () => {
      ...
      const processedConversations =
        data.conversations?.map((convo: Conversation) => {
        ...
        for (const tempConv of tempConvs) {
          ...
          const existingConvo = mergedConversations.find((convo) =>
            convo.participants.some(
              (p: { user_id: string }) => p.user_id === tutorId
            )
          );
    ...
    async (conversationId: string, forceRefresh = false) => {
      ...
      try {
        ...
        const paginationParam =
          currentMessages.length > 0 && forceRefresh
            ? `&before=${currentMessages[0].id}&limit=${limit}`
            : `&limit=${limit}`;

        const response = await fetch(
          `/api/messages?conversation_id=${conversationId}${paginationParam}`,
          {
            credentials: "include",
          }
        );
    ...
    async (
      ...
      if (isTemp) {
        try {
          ...
          verificationSuccessful = await verifyConversationReady(
          ...
          if (isCsrfError) {
            ...
            try {
              await fetchCsrfToken();
              throw new Error(
                "CSRF token error - please try again. The token has been refreshed."
              );
            } catch (refreshError) {
              throw new Error(
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt > 0) {
            ...
            const isNewUnverifiedConversation =
              isTemp && !verificationSuccessful;
            ...
            try {
              ...
              const notificationMessage = {
                ...
                recipient: tempConversations[conversationId]
                  ? {
                      ...
                      display_name:
                        tempConversations[conversationId].tutorName || "Tutor",
                      avatar_url:
                        tempConversations[conversationId].tutorAvatar || null,
                      is_tutor: true,
                    }
                  : undefined,
                // Add any fields the notification system needs
                created_at:
                  serverMessage.created_at || new Date().toISOString(),
                sender_id: user.id,
              };
              ...
              setTempConversations((prev) => {
                ...
                try {
                  localStorage.setItem(
                    "tempConversations",
                    JSON.stringify(newTempConversations)
                  );
                } catch (e) {}
              ...
              setConversations((prevConversations) => {
                ...
                const filteredConvos = prevConversations.filter(
                  (convo) => convo.id !== conversationId
                );

                // Make sure the real conversation is included
                const hasRealConvo = filteredConvos.some(
                  (convo) => convo.id === actualConversationId
                );
                if (!hasRealConvo) {
                    ...
                    participants: [
                      {
                        ...
                        user: {
                          ...
                          last_name: user.name?.split(" ")[1] || "",
                          avatar_url:
                            user.profilePic || user.avatar_url || null,
                          is_tutor: user.role === "tutor",
                        },
                      },
                      {
          ...
          setMessages((prev) => {
            ...
            if (messageIndex >= 0) {
              ...
              const duplicateIndex = conversationMessages.findIndex(
                (msg, idx) =>
                  idx !== messageIndex && msg.id === serverMessage.id
              );
              ...
              const existingServerMsg = conversationMessages.findIndex(
              ...
              if (messageIndex >= 0) {
                ...
                if (error instanceof Error) {
                  if (error.message.includes("conversation")) {
                    errorMessage =
                      " (Conversation creation failed, please try again)";
                  } else if (
    ...
    if (typeof window !== "undefined") {
      try {
        if (Object.keys(tempConversations).length > 0) {
          localStorage.setItem(
            "tempConversations",
            JSON.stringify(tempConversations)
          );
        } else {
