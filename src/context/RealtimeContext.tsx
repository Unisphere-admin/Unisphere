...
export const RealtimeProvider = ({ children }: { children: ReactNode }) => {
  ...
  const broadcastSessionUpdate = useCallback((session: RealtimeSession) => {
    console.log('[REALTIME DEBUG] Broadcasting session update:', {
    ...
    try {
      // Store the session update in localStorage for tracking
      const key = `session_update_${Date.now()}`;
      ...
      console.log('[REALTIME DEBUG] Session update saved to localStorage:', key);
      ...
      console.error('[REALTIME DEBUG] Error broadcasting session update:', error);
    }
  }, []);
